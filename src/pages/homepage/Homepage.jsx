...
export default function Homepage() {
    ...
    useEffect(() => {
        ...
        const renderCanvas = (newT) => {
            ...
            const points = [
                [
                    x(-100 - Math.sin(animationX * Math.log(9)) * 50),
                    y(60 - Math.sin(animationX / 1.5) * 30),
                ],
                [
                    x(100 + Math.sin(animationX / 5) * 50),
                    y(60 + Math.sin((animationX * Math.E) / Math.PI) * 60),
                ],
                [
                    x(200 - Math.sin(animationX / 3) * 50),
                    y(60 - Math.sin((animationX * Math.PI) / Math.E) * 60),
                ],
                [
                    x(400 - Math.sin(animationX * Math.log(11)) * 50),
                    y(60 + Math.sin(animationX / 1.3) * 30),
                ],
            ];
            ...
            ctx.beginPath();
            ctx.moveTo(
                points[0][0] + x(Math.sin(animationX / 0.8) * 40),
                points[0][1] + y(40)
            );
            ctx.bezierCurveTo(
                points[1][0] + x(Math.sin(animationX / 0.9) * 40),
                points[1][1] + y(40),
                points[2][0] + x(Math.sin(animationX / 1.1) * -40),
                points[2][1] + y(40),
                points[3][0] + x(Math.sin(animationX / 1.2) * -40),
                points[3][1] + y(40)
            );
            ctx.stroke();
            ...
            for (let i = -2; i < lineCount; i++) {
                ctx.beginPath();
                ctx.moveTo(x(i * linePadding + animationOffset), y(0));
                ctx.lineTo(x((i - 1) * linePadding + animationOffset), y(150));
                ctx.stroke();
            }
